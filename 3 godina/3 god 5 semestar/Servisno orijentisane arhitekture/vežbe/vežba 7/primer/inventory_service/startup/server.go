...
import (
	...
	handlers2 "github.com/tamararankovic/microservices_demo/inventory_service/handlers"
	"github.com/tamararankovic/microservices_demo/inventory_service/startup/config"
	store2 "github.com/tamararankovic/microservices_demo/inventory_service/store"
...
func (server *Server) initProductStore(client *gorm.DB) domain.ProductStore {
	store, err := store2.NewProductPostgresStore(client)
	if err != nil {
		log.Fatal(err)
	}
	store.DeleteAll()
...
func (server *Server) initCreateOrderHandler(service *application.ProductService, publisher saga.Publisher, subscriber saga.Subscriber) {
	_, err := handlers2.NewCreateOrderCommandHandler(service, publisher, subscriber)
	if err != nil {
		log.Fatal(err)
	}
}

func (server *Server) initProductHandler(service *application.ProductService) *handlers2.ProductHandler {
	return handlers2.NewProductHandler(service)
}

func (server *Server) startGrpcServer(productHandler *handlers2.ProductHandler) {
