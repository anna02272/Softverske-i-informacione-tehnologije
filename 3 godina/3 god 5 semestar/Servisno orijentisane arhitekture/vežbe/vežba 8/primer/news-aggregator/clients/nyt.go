...
func NewNYTClient(client *http.Client, address string, cb *gobreaker.CircuitBreaker) domain.NewsSource {
	return &NYTClient{
		client:  client,
		address: address,
		cb:      cb,
...
func (c *NYTClient) Search(ctx context.Context, query string) ([]domain.Article, error) {
	...
	err := retrier.RunCtx(ctx, func(ctx context.Context) error {
		...
		cbResp, err := c.cb.Execute(func() (interface{}, error) {
			...
			// if we have a timeout for the request set
			// we let the upstream service be aware of it
			// so that it can stop processing the request
	...
	return toDomainArticles(searchResp, "NYT"), nil
}
