...
func main() {
	...
	productsHandler := handlers.NewProductsHandler(logger, store)
	...
	getByTypeRouter := router.Methods(http.MethodGet).Subrouter()
	getByTypeRouter.HandleFunc("/type/{type}", productsHandler.GetProductsByType)
	...
	// NoSQL: removed int validation because we switched to uuid
