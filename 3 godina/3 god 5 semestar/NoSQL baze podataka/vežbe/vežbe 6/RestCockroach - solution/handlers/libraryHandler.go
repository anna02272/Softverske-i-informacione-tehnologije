...
func (l *LibraryHandler) GetBooksByAuthor(rw http.ResponseWriter, h *http.Request) {
	vars := mux.Vars(h)
	authorId := vars["authorId"]
	l.logger.Println(authorId)

	books, err := l.repo.GetBooksByAuthor(authorId)
	if err != nil {
		l.logger.Print("Database exception: ", err)
		rw.WriteHeader(http.StatusBadRequest)
		return
...
func (l *LibraryHandler) ReturnBook(rw http.ResponseWriter, h *http.Request) {
	vars := mux.Vars(h)
	isbn := vars["isbn"]
	memberId := vars["memberId"]
	err := l.repo.ReturnBook(isbn, memberId)
	if err != nil {
		l.logger.Print("Database exception: ", err)
		rw.WriteHeader(http.StatusBadRequest)
		return
...
func (l *LibraryHandler) GetBorrowingsByMember(rw http.ResponseWriter, h *http.Request) {
	vars := mux.Vars(h)
	memberId := vars["memberId"]
	borrowings, err := l.repo.GetBorrowingsByMember(memberId)
