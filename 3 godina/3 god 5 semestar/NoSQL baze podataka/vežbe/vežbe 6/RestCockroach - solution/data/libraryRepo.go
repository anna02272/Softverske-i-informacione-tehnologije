...
func (lr *LibraryRepo) GetBooksByAuthor(authorId string) (Books, error) {
	var books Books
	err := lr.db.Where("author_id = ?", authorId).Find(&books).Error
	if err != nil {
		return nil, err
	}
	return books, nil
...
func (lr *LibraryRepo) GetBorrowingsByMember(memberId string) (Borrowings, error) {
	var borrowings Borrowings
	err := lr.db.Joins("JOIN members ON borrowings.member_id = members.id").
...
func (lr *LibraryRepo) ReturnBook(isbn string, memberId string) error {
	// Begin a new transaction
	tx := lr.db.Begin()
	...
	// Check if the book is borrowed by specified member
	var borrowing Borrowing
	// SQL syntax for PostgreSQL (which CockroachDB is compatible with) allows bool comparison in couple of ways:
	// TRUE, 't', 'true', 'y', 'yes', 'on', '1'
	// as well as the not syntax as seen below
	if err := tx.Where("book_isbn = ? and member_id = ? and not is_returned", isbn, memberId).First(&borrowing).Error; err != nil {
		tx.Rollback()
		return err
	}

	// Update the Borrowing as returned
