    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (Priznanica priznanica in korisnik.Priznanice)
            {
                ...
                {
                    double iznosZaIspis = 0;
                    
                    if (stavka is UtrosakGrejanja)
                        iznosZaIspis = ((UtrosakGrejanja)stavka).IzracunajIznos();
                    else if (stavka is UtrosakVode)
                        iznosZaIspis = ((UtrosakVode)stavka).IzracunajIznos();
                    
                    Console.WriteLine("\t" + stavka.Naziv + ":" + iznosZaIspis.ToString());
                }
            }
        }
    }
