...
public class KorisnikDAOImpl implements KorisnikDAO{
	...
	public int save(Korisnik korisnik) {
		...
		return jdbcTemplate.update(sql, korisnik.getKorisnickoIme(), korisnik.getLozinka(), korisnik.getEMail(), korisnik.getPol(), korisnik.isAdministrator());
	}
	@Override
	public int update(Korisnik korisnik) {
		if (korisnik.getLozinka() == null) {
			String sql = "UPDATE korisnici SET eMail = ?, pol = ?, administrator = ? WHERE korisnickoIme = ?";
			return jdbcTemplate.update(sql, korisnik.getEMail(), korisnik.getPol(), korisnik.isAdministrator(), korisnik.getKorisnickoIme());
		} else {
			String sql = "UPDATE korisnici SET lozinka = ?, eMail = ?, pol = ?, administrator = ? WHERE korisnickoIme = ?";
			return jdbcTemplate.update(sql, korisnik.getLozinka(), korisnik.getEMail(), korisnik.getPol(), korisnik.isAdministrator(), korisnik.getKorisnickoIme());
		}
	}
	@Override
