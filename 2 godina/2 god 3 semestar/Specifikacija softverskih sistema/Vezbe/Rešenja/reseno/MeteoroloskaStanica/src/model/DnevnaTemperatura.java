...
/**
 * DnevnaTemperatura - Klasa koja čuva izmerene dnevne temeraturu na jednom
...
public class DnevnaTemperatura implements Publisher {
	...
	public DnevnaTemperatura(Date datum, float minVrednost, float maxVrednost) {
		super();
		this.datum = datum;
	...
	public void setMinVrednost(float minVrednost) {
		this.minVrednost = minVrednost;
	}

	public void setMaxVrednost(float maxVrednost) {
		this.maxVrednost = maxVrednost;
	}

	/**
	 * dodajTemperaturu - metoda za dodavanje temperature u listu temperatura, uz
	 * različite provere
	 */
	public void dodajTemperaturu(float temperatura) {
		if (null == izmereneVrednosti) {
			izmereneVrednosti = new ArrayList<Float>(); // lazy inicijalizacija - kreiramo listu samo kada za njom
														// postoji potreba
		}
		if (temperatura > MAX_TEMP || temperatura < MIN_TEMP)
			throw new IllegalArgumentException(
					String.format("Temperatura mora biti u opsegu [%f, %f]", MIN_TEMP, MAX_TEMP));
	...
	/**
	 * List<Observer> observers - lista onih koji "osluskuju" odnosno posmatraju
	 * promenu podataka (Observer pattern) Više razlicitih elemenata korisnickog
	 * interfejsa (cak i oni koji nisu poznati u trenutku inicijalnog dizajna
	 * aplikacije) se mogu registrovati da slusaju promene
	 */
	...
	/**
	 * notifyObservers - Metoda za slanje dogadjaja da se desila promena svima koji
	 * su se registrovali za pracenje promena
	 */
	@Override
